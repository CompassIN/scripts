...
void runlogic()
{
    ...
    NSDL2_RUNLOGIC(NULL, NULL, "Executing percent block - Block1");
    {
        ...
        if(Block1percent <= 2500)
        {
            NSDL2_RUNLOGIC(NULL, NULL, "Executing flow - flow (pct value = 25.0%)");
            UPDATE_USER_FLOW_COUNT(1)
            flow();
        }
        else if(Block1percent <= 10000)
        {
            NSDL2_RUNLOGIC(NULL, NULL, "Executing flow - Login (pct value = 75.0%)");
            UPDATE_USER_FLOW_COUNT(63)
            Login();
        }
    }
